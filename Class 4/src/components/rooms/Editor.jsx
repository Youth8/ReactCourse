...
class RoomEditor extends BaseComponent {
  constructor(props, context) {
    ...
    if (match.params.id && this.props.rooms && this.props.rooms.length > 0) {
      const tRoom = logic.findRoom(this.props.rooms, match.params.id);
      if (tRoom.i > -1) {
  ...
  onSubmit() {
    ...
    if (valid) {
      ...
      if (id) {
        newRoom = logic.findRoom(this.props.rooms, id);
        if (newRoom.i > -1) {
        ...
        this.props.changeRooms([...newRooms]);
      } else {
        this.props.changeRooms([...rooms, newRoom]);
      }
